	...
	if receiverType.Kind() == reflect.Ptr {
		...
		if elem.Kind() == reflect.Struct {
			...
			for i := 0; i < s.NumField(); i++ {
				...
				// search in direct dependencies first
				if dependencies != nil {
					depValue := reflect.ValueOf(dependencies)

					if depValue.Kind() == reflect.Ptr && depValue.Elem().Kind() == reflect.Struct {
						ds := depValue.Elem()

						for i := 0; i < ds.NumField(); i++ {
							df := ds.Field(i)

							if df.IsValid() && df.Type() == f.Type() {
						...
						return fmt.Errorf("dependencies is not a pointer to a struct, got: %v", depValue.Kind())
					}
				}
				...
				if concrete, exist := c[f.Type()][name]; exist {
					...
					if f.CanAddr() {
						err := c.DeepFill(ptr.Addr().Interface(), dependencies)
						if err != nil {
							return err
						}
					} else {
						return fmt.Errorf("container: %v field is not addressable", s.Type().Field(i).Name)
					}
				}
			}
